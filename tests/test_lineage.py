...
def simulate_snps(ind, chrom='1', pos_start=1, pos_max=248140902, pos_step=100, genotype='AA',
    ...
    if complement_genotype_two_chroms:
        snps.loc[indices, 'genotype'] = snps.loc[indices, 'genotype'].apply(complement_two_chroms)
    elif complement_genotype_one_chrom:
        snps.loc[indices, 'genotype'] = snps.loc[indices, 'genotype'].apply(complement_one_chrom)
...
def test_find_discordant_snps(l):
    df = pd.read_csv('tests/input/discordant_snps.csv', skiprows=1, na_values='--',
    ...
    pd.testing.assert_index_equal(pd.Index(['rs34', 'rs47', 'rs48', 'rs49', 'rs50', 'rs106',
                                            'rs110', 'rs111', 'rs112', 'rs113', 'rs137', 'rs140',
                                            'rs141', 'rs144', 'rs146', 'rs147'], name='rsid'),
                                  df_ind1_ind2.index)

    pd.testing.assert_index_equal(pd.Index(['rs30', 'rs34', 'rs38', 'rs42', 'rs46', 'rs47',
                                            'rs48', 'rs49', 'rs50', 'rs60', 'rs75', 'rs85',
                                            'rs100', 'rs102', 'rs106', 'rs110', 'rs111', 'rs112',
                                            'rs113', 'rs114', 'rs118', 'rs122', 'rs135', 'rs137',
                                            'rs139', 'rs140', 'rs141', 'rs142', 'rs144', 'rs146',
...
def test_find_shared_dna_no_shared_dna(l):
    ind1 = simulate_snps(l.create_individual('ind1'))
    ind2 = simulate_snps(l.create_individual('ind2'), complement_genotype_two_chroms=True)
