...
class Individual(object):
    ...
    def save_snps(self, filename=None):
        ...
        """
        comment = self._generate_header_comment()

        comment += '# Assembly: {}\n' \
                   '# SNPs: {}\n' \
                   '# Chromosomes: {}\n'

        comment = comment.format(self.assembly,
                                 self.snp_count,
    ...
    def save_discrepant_positions(self, filename=None):
        ...
        """
        if filename is None:
            filename = self.get_var_name() + '_discrepant_positions.csv'

        return lineage.save_df_as_csv(self.discrepant_positions, self._output_dir,
