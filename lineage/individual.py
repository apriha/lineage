...
class Individual(object):
    ...
    def save_discrepant_positions(self, filename=None):
        ...
        str
            path to file in output directory if SNPs were saved, else empty str
        """
        return self._save_discrepant_snps(self.discrepant_positions, 'discrepant positions',
                                          filename)
    ...
    def save_discrepant_genotypes(self, filename=None):
        ...
        str
            path to file in output directory if SNPs were saved, else empty str
        """
        return self._save_discrepant_snps(self.discrepant_genotypes, 'discrepant genotypes',
                                          filename)

    def _save_discrepant_snps(self, df, name_discrepant_snps, filename=None):
        """ Save discrepant SNPs to file.

        Parameters
        ----------
        df : pandas.DataFrame
            discrepant SNPs
        name_discrepant_snps : str
            name of discrepant SNPs to save
        filename : str
            filename for file to save
        ...
        if len(df) > 0:
            try:
                ...
                if filename is not None:
                    ...
                    file = os.path.join(output_dir, self._clean_string(name_discrepant_snps) +
                                        '_' + self.get_var_name() + '.csv')
            ...
            print('no {} to save...'.format(name_discrepant_snps))
            return ''
